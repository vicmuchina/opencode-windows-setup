...
For sites with reCAPTCHA or authentication requirements, use the pre-configured Chrome profile:

### Step 1: Start Chrome with the profile

```bash
bash /home/vic/fix-chrome-debug.sh
```

This script:
- Kills any existing Chrome processes
- Starts Chrome with remote debugging on port 9333
- Uses the profile at `/home/vic/.config/chrome-mcp-debug/`
...
- **Profile Location**: `/home/vic/.config/chrome-mcp-debug/`
- **Debug Port**: 9333
- **Benefits**:
  - Persistent cookies and sessions
  - Bypasses reCAPTCHA challenges
  - Maintains login state across sessions
  - Uses your browser fingerprint

### Manual Chrome Start (Alternative)

If the script doesn't work, start Chrome manually:

```bash
CHROME_USER_DATA_DIR="/home/vic/.config/chrome-mcp-debug"
REMOTE_DEBUG_PORT=9333

rm -f "$CHROME_USER_DATA_DIR/SingletonLock" "$CHROME_USER_DATA_DIR/SingletonCookie" "$CHROME_USER_DATA_DIR/SingletonSocket" 2>/dev/null
...
Then connect with: `agent-browser connect 9333`
